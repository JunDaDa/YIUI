...
namespace ET
{
    public struct NumericHandlerDynamicInfo
    {
        ...
        public int InvokeParentLayerCount { get; } //非0时精准响应

        public NumericHandlerDynamicInfo(int sceneType, INumericHandlerDynamic numericHandler, int invokeParentLayerCount = 0)
        {
            this.SceneType = sceneType;
            this.INumericHandler = numericHandler;
            this.InvokeParentLayerCount = invokeParentLayerCount;
        }
    }
    ...
    public class NumericHandlerDynamicComponent : Singleton<NumericHandlerDynamicComponent>, ISingletonAwake
    {
        ...
        public void Awake()
        {
            ...
            foreach (var type in types)
            {
                ...
                if (!tTypeDic.ContainsKey(numericType))
                {
                    tTypeDic.Add(numericType, new List<NumericHandlerDynamicInfo>());
                }

                tTypeDic[numericType].Add(numericHandlerInfo);
                GetSystemType(tType2);
                AddTypeCount(tType2, numericType);
            }
        }
        ...
        private async ETTask Run(NumericChange data, int runNumericType)
        {
            ...
            if (changeEntity == null || changeEntity.IsDisposed) return;
            ...
            EntityRef<Entity> changeEntityRef = changeEntity;
            using var hashSet = HashSetComponent<Entity>.Create();
            while (count-- > 0)
            {
                if (changeEntityRef.Entity == null)
                {
                    continue;
                }
                ...
                {
                    // >0 说明需要精准响应
                    if (numericHandler.InvokeParentLayerCount > 0)
                    {
                        Entity checkEntity = component;
                        for (int i = 0; i < numericHandler.InvokeParentLayerCount; i++)
                        {

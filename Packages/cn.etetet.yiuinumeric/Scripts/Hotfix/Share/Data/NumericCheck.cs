namespace ET
{
    ...
    public static class NumericCheck
    {
        ...
        //检查是否包含 0-6中的 结果0
        public static bool CheckContain0(this ENumericType numericType)
        {
            return CheckContain0((int)numericType);
        }

        public static bool CheckContain0(this int numericId)
        {
            //[Min - Max] = 这个数值的最终值 是不允许直接修改的
            if (numericId is >= NumericConst.Min and <= NumericConst.Max)
            {
        ...
        /// 获取目标数值的 公式ID
        /// </summary>
        public static int GetNumericFormulaId(this ENumericType numericType)
        {
            var checkConfig = numericType.GetCheckConfig();
            if (checkConfig == null)
            {
                Log.Error($"没有找到这个 [{numericType}] 的配置");
                return 0;
            }

            return checkConfig.FormulaId;
        }

        //获取目标数值的 公式ID
        public static int GetNumericFormulaId(this int numericType)
        {
            return GetNumericFormulaId((ENumericType)numericType);
        }
    }
}